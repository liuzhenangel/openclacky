...
const Skills = (() => {
  ...
  return {
    ...
    async installFromStore(name, title, repo) {
      ...
      // Use setPendingMessage (same pattern as createInSession/channels) so the message
      // is sent only after WS "subscribed" is confirmed. This avoids a race where
      // loadHistory fires after the server has already saved the message, causing
      // the user bubble to appear twice (once from appendMsg, once from history replay).
      Sessions.setPendingMessage(session.id, msg);
