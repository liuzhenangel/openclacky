...
const Router = (() => {
  ...
  function _apply(view, params = {}) {
    ...
    switch (view) {
      ...
      case "brand":
        // No hash — keep URL clean during brand activation
        $("brand-panel").style.display = "flex";
        break;
...
// Boot sequence:
//   1. Brand check    — license activation for white-label installs (MUST be first)
//   2. Onboard check  — first-run key setup / soul setup
//   3. Normal UI boot — WS + sessions + tasks + skills
//
// Brand activation is checked first so the license gate is always presented
// before anything else. If the user skips brand activation, a dismissible
// warning bar is shown and the boot continues normally.
// Brand._skipActivation() and Brand._doActivate() both call window.bootAfterBrand()
// to continue the sequence.
//
// Only "key_setup" is a hard block for onboard (nothing works without an API key).
...
(async () => {
  ...
  if (needsBrandActivation) return;  // Brand will call bootAfterBrand() when done
