...
const Sessions = (() => {
  ...
  let   _activeId          = null;
  let   _activeTab         = "manual";  // "manual" | "cron" | "channel" | "system"
  // Per-source pagination state (manual/cron/channel/system for general tabs; coding for coding section)
  const _hasMoreBySource   = { manual: false, cron: false, channel: false, coding: false, system: false };
  const _loadingMoreSource = { manual: false, cron: false, channel: false, coding: false, system: false };
  ...
  return {
     ...
     *  bucket: "manual" | "cron" | "channel" | "system"  → uses ?source=<bucket>
    ...
    async loadMoreSessions(source) {
      ...
      try {
        ...
        const param = source === "coding" ? "profile=coding" : `source=${encodeURIComponent(source)}`;
    ...
    renderList() {
      ...
      const hasSetup   = generalSessions.some(s => s.source === "system")  || _hasMoreBySource["system"];
      ...
      // Fallback: if active tab's content is gone, revert to manual
      let effectiveTab = _activeTab;
      if (effectiveTab === "cron"    && !hasCron)    effectiveTab = "manual";
      if (effectiveTab === "channel" && !hasChannel) effectiveTab = "manual";
      if (effectiveTab === "system"  && !hasSetup)   effectiveTab = "manual";
